...
const SingleProduct = (props) => {
    ...
    <Fragment>
              ...
              <div className="relative col-span-1 my-3 mx-[0.9rem] ">
                ...
                {item.pOffer !== 0 && item.pOffer !== "0" ? (
                  <div className="md:text-[19px] font-semibold text-[#a1510c] text-base">
                    {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item.pPrice)}
                    <span className="md:text-[15px] text-[13px] relative top-[1px] right-1 sm:right-[0px] line-through p-1 text-black">
                      {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item.pOffer)}
                    </span>{" "}
                  </div>
                ) : (
                  <div className="md:text-[19px] font-semibold text-[#a1510c] text-base">
      ...
      <div className="text-center mb-12 mt-16 md:mt-20 lg:mt-24">
        <button
            ...
            style={{ backgroundColor: currentPage === 1 ? '#A1510C' : '' }}
          >
            <span style={{ color: currentPage === 1 ? 'white' : '' }}>Previous</span>
        </button>
        <button
            onClick={() => setCurrentPage(currentPage + 1)}
            ...
            style={{ backgroundColor: currentPage === totalPages ? '#A1510C' : '' }}
          >
           <span style={{ color: currentPage === totalPages ? 'white' : '' }}>Next</span>
        </button>
      </div>
      }
  ...
  <div className="fixed bottom-4 right-4">
    <button
      ...
      className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-medium py-2 px-4 rounded cursor-pointer"
      style={{ backgroundColor: "#a1510c", color: "#ffffff" }}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        className="h-6 w-6"
