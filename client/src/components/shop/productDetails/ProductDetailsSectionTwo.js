  ...
  return (
    <Fragment>
      <div className="flex flex-col md:flex-row items-center justify-center">
        <div
          onClick={(e) => dispatch({ type: "menu", payload: 'mo_ta' })}
          className={`${data.menu === "mo_ta" ? "border-b-2 border-yellow-700" : ""
            } px-4 py-3 cursor-pointer`}
        >
          Mô tả
        </div>
        <div
          onClick={(e) => dispatch({ type: "menu", payload: 'specs' })}
          className={`${data.menu === "specs" ? "border-b-2 border-yellow-700" : ""
            } px-4 py-3 cursor-pointer`}
        >
          Thông số kỹ thuật
        </div>
        <div
          onClick={(e) => dispatch({ type: "menu", payload: 'faq' })}
          className={`${data.menu === "faq" ? "border-b-2 border-yellow-700" : ""
            } px-4 py-3 cursor-pointer`}
        >
          Câu hỏi thường gặp
        </div>
