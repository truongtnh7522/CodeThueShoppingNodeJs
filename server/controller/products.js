...
static deleteImages(images, mode) {
  ...
  async getDeleteProduct(req, res) {
    ...
    if (!pId) {
      ...
      try {
        // Check if any orders are associated with the product
        let orders = await orderModel.find({ "allProduct.id": pId });
        if (orders.length > 0) {
          return res.json({
            error:
              "This product is included in an order, it cannot be deleted.",
          });
        } else {
